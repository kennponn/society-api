...
import society.bean.JsonResult;
import society.bean.User;
import society.mapper.UserMapper;
...
public class UserController {
	...
	public ResponseEntity<JsonResult> getUserList() {
		JsonResult r = new JsonResult();
		try {
			r.setResult(userMapper.selectAllUsers());
			r.setStatus("ok");
		} catch (Exception e) {
			r.setResult(e.getClass().getName() + ":" + e.getMessage());
			r.setStatus("error");
	...
	public ResponseEntity<JsonResult> update(@PathVariable("id") Integer id, @RequestBody User user) {
		...
		try {
			...
			u.setCtm(user.getCtm());
			u.setAvator(user.getAvator());
			u.setSummary(user.getSummary());
			u.setVoiceSummary(user.getVoiceSummary());
